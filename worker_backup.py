...
def hisse_analiz_et(symbol):
    try:
        ...
        with contextlib.redirect_stderr(f):
            ...
            # ─── GERÇEK ZAMANLI FİYAT: fast_info'dan al ───
            # history() son mumun kapanışını verir (eski olabilir).
            # fast_info['lastPrice'] her zaman en güncel fiyatı döner.
            try:
                realtime_price = float(hisse.fast_info.get('lastPrice') or 0)
            except:
                realtime_price = 0.0
...
def guncel_bist_hisseleri_getir():
    try:
        ...
        if response.status_code == 200:
            import re
            # SADECE Hisse-Detay.aspx?hisse=XXXX linklerini yakala (yanlış eşleşmeleri engelle)
            matches = re.findall(r'Hisse-Detay\.aspx\?hisse=([A-Z0-9]{4,5})"', response.text)
            hisseler = list(set(matches))
            hisseler = [h + ".IS" for h in hisseler]
            # BIST'te max ~550 hisse var; makul sınır 40-600 arası
            if 40 <= len(hisseler) <= 600:
                print(f"✅ Anlık BIST hisse listesi çekildi: {len(hisseler)} hisse")
                return hisseler
            elif len(hisseler) > 600:
                hisseler = sorted(hisseler)[:550]
                print(f"✅ BIST listesi 550 ile sınırlandırıldı.")
                return hisseler
    except Exception as e:
...
def taramayi_baslat():
    ...
    while True:
        ...
        # max_workers=8: Yahoo ban riski dengede tutularak hız artırıldı.
        # Her tamamlanan hisse arasında 0.1s bekleme (toplamda ~0.1s/thread).
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_symbol):
                ...
                # Yahoo Finance 429 koruması (önceki 0.3s → 0.1s)
                time.sleep(0.1)
        ...
        time.sleep(90)  # Döngü bekleme: 3dk → 90sn (daha sık güncelleme)
